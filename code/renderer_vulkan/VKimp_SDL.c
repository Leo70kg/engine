...
static void GL_InitInstance( void )
{
	...
	if(!SDL_Vulkan_GetInstanceExtensions(SDL_window, &sdl_extension_count, NULL))
		ri.Error(ERR_FATAL, "SDL_Vulkan_GetInstanceExtensions failed: %s", SDL_GetError());
	...
	application_info.pApplicationName = "vkOpenArena";
	application_info.applicationVersion = 1;
	application_info.pEngineName = "vkOpenArena";
	...
	err = vkCreateInstance(&instance_create_info, NULL, &vk.instance);
	if (err != VK_SUCCESS)
		ri.Error(ERR_FATAL, "Couldn't create Vulkan instance");

	if (!SDL_Vulkan_CreateSurface(SDL_window, vk.instance, &vk.surface))
		ri.Error(ERR_FATAL, "Couldn't create Vulkan surface");
    ...
    #define GET_INSTANCE_PROC_ADDR(inst, entrypoint) { \
	    ...
	    if (fp##entrypoint == NULL) ri.Error(ERR_FATAL, "vkGetInstanceProcAddr failed to find vk" #entrypoint); \
    }
	...
	GET_INSTANCE_PROC_ADDR(vk.instance, GetPhysicalDeviceSurfacePresentModesKHR);
	GET_INSTANCE_PROC_ADDR(vk.instance, GetSwapchainImagesKHR);

#ifdef _DEBUG
	if(vulkan_globals.validation)
	{
		Con_Printf("Creating debug report callback\n");
		GET_INSTANCE_PROC_ADDR(vk.instance, CreateDebugReportCallbackEXT);
		GET_INSTANCE_PROC_ADDR(vk.instance, DestroyDebugReportCallbackEXT);
		...
		err = fpCreateDebugReportCallbackEXT(vk.instance, &report_callback_Info, NULL, &debug_report_callback);
		if (err != VK_SUCCESS)
